...
func (w *Wrapper[T]) Equals(v T) bool {
	return UniversalEquals(w.Value, v).GetValue()
}
...
func (w *Wrapper[T]) LessThan(v T) bool {
    return UniversalCompare(w.Value, v) == compare.Less
}
...
func (w *Wrapper[T]) GreaterThan(v T) bool {
    return UniversalCompare(w.Value, v) == compare.Greater
}
